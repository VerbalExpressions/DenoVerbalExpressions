...
/**
 * @file VerbalExpressions TypeScript/Deno Library Adaptation
 * @version 1.0.0
 * @license MIT
 *
 * @see https://github.com/VerbalExpressions/JSVerbalExpressions
 *
 * Ported to Deno by Shawn Bullock.  github.com/ievolved
 *
 */

export type RegExpFlags = "g" | "i" | "m" | "u" | "y";
export type Inputs = VerbalExpression | RegExp | string | number;
 ...
 * @extends {RegExp}
 */
export class VerbalExpression extends RegExp {
   ...
   * @memberof VerbalExpression
   */
  static sanitize(value: Inputs): string {
   ...
   * @memberof VerbalExpression
   */
  startOfLine(enable: boolean = true): VerbalExpression {
   ...
   * @memberof VerbalExpression
   */
  endOfLine(enable: boolean = true): VerbalExpression {
   ...
   * @memberof VerbalExpression
   */
  then(value: Inputs): VerbalExpression | any {
   ...
   * @memberof VerbalExpression
   */
  maybe(value: Inputs): VerbalExpression {
    value = VerbalExpression.sanitize(value);
    return this.add(`(?:${value})?`);
  }
   ...
   * @param {(string|RegExp|number)} value to find
   * @returns {VerbalExpression} recompiled instance of VerbalExpression
   * @memberof VerbalExpression
   */
  or(value?: Inputs): VerbalExpression {
    this._prefixes += '(?:';
   ...
   * @memberof VerbalExpression
   */
  any(value: Inputs): VerbalExpression {
   ...
   * @memberof VerbalExpression
   */
  not(value: Inputs): VerbalExpression {
    value = VerbalExpression.sanitize(value);
    this.add(`(?!${value})`);
   ...
   * @memberof VerbalExpression
   */
  withAnyCase(enable: boolean = true): VerbalExpression {
   ...
   * @memberof VerbalExpression
   */
  stopAtFirst(enable: boolean = true): VerbalExpression {
   ...
   * @memberof VerbalExpression
   */
  searchOneLine(enable: boolean = true): VerbalExpression {
  ...
  repeatPrevious(...quantity: number[]): VerbalExpression {
    const isInteger = /\d+/;
    const values = quantity.filter(argument => isInteger.test( String(argument) ));
  ...
  multiple(value: string, lower: number, upper?: number): VerbalExpression {
    if (value !== undefined) {
      value = VerbalExpression.sanitize(value) as string;
      this.add(`(?:${value})`);
    }
 ...
 */
export function VerEx() : VerbalExpression | any { // eslint-disable-line no-unused-vars
